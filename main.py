...
#: Genre BPM windows. Defined once in ``agent/genres.py`` and exposed here
#: under its historical name — this used to be a literal duplicated in
#: ``agent/tools.py``, and the two drifted. It is a LIVE view: a genre this
#: installation added from the catalog UI appears without a restart.
from agent import genres as _genres  # noqa: E402

BPM_GENRE_RANGES = _genres.BPM_RANGES

# Default themes per genre folder for smart-generated sessions
#: Per-genre visual identity. Defined once in ``agent/genres.py``; see
#: ``BPM_GENRE_RANGES`` above for why this is a view and not a literal.
