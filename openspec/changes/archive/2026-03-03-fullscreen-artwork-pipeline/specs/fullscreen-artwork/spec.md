...
The system SHALL scale artwork images using a cover-crop algorithm that fills the entire target frame (1920×1080) with zero black or white borders, preserving the image's original aspect ratio.

#### Scenario: DALL-E artwork at 1792×1024
- **WHEN** artwork generated at 1792×1024 is loaded for video background
- **THEN** the image is scaled up to cover 1920×1080 and center-cropped, with no letterboxing or pillarboxing

#### Scenario: Square artwork input
- **WHEN** artwork with a 1:1 aspect ratio is loaded
- **THEN** the image is scaled to cover the full 1920×1080 frame (cropping top/bottom excess) with no borders
